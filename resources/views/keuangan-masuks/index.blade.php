        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <div class="p-6 bg-white border-b border-gray-200 overflow-x-auto">
                        ...
                        <tbody class="text-gray-600 text-sm divide-y divide-gray-200">
                            @forelse ($keuanganMasuks as $index => $item)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="p-3 font-semibold text-gray-800">{{ $item->nama_produk }}</td>
                                    <td class="p-3 text-center">{{ $item->quantity }}</td>
                                    <td class="p-3 text-right whitespace-nowrap">Rp {{ number_format($item->harga_satuan, 0, ',', '.') }}</td>
                                    <td class="p-3 text-right whitespace-nowrap font-bold text-green-600">Rp {{ number_format($item->jumlah, 0, ',', '.') }}</td>
                                    ...
                                    <td class="p-3 text-center whitespace-nowrap">
                                        <div class="flex gap-2 justify-center">
                                            ...
                                            <form action="{{ route('keuangan-masuks.destroy', $item->id) }}" method="POST" 
                    ...
                    @if(method_exists($keuanganMasuks, 'links'))
                        <div class="mt-4">
                            {{ $keuanganMasuks->links() }}
                        </div>
                    @endif
