    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('keuangan-masuks.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_produk" value="{{ old('nama_produk') }}" placeholder="Contoh: Penjualan Kaos"
                            class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        @error('nama_produk')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="number" name="quantity" id="quantity" value="{{ old('quantity') }}" min="1" placeholder="0"
                                class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            @error('quantity')
                                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="harga_satuan" id="harga_satuan" value="{{ old('harga_satuan') }}" min="0" placeholder="0"
                                class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            @error('harga_satuan')
                                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah') }}" readonly placeholder="Otomatis terhitung"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="keterangan" rows="3" placeholder="Tambahkan catatan jika diperlukan..."
                            class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
