    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('keuangan-keluars.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama" value="{{ old('nama') }}" placeholder="Contoh: Pembelian Alat Tulis"
                            class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-red-500 focus:outline-none">
                        @error('nama')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="keterangan" rows="3" placeholder="Tambahkan catatan atau detail pengeluaran..."
                            class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-red-500 focus:outline-none">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
